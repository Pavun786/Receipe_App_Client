...
export const ViewRecipe = () => {
  ...
  useEffect(() => {
    ...
    try {
      const recipeResponse = await axios.get(`http://localhost:5000/recipes/${recipeID}`);
      const recipeData = recipeResponse.data;
      setRecipe(recipeData);

      // Fetch display name of the recipe-created-user
      // const ownerNameresponse = await axios.get(`http://localhost:5000/auth/${recipeData.recipeOwner}`);
